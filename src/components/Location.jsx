    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid lg:grid-cols-12 gap-10 items-center">
                ...
                return (
                  <div
                    ...
                    className={`cursor-pointer transition-all duration-500
                      ${
                        isActive
                          ? "translate-x-4 scale-105 z-20"
                          : "opacity-70 hover:opacity-100 hover:translate-x-2"
                      }`}
                  >
                    <div
                      className={`p-5 rounded-3xl flex items-center gap-4 shadow-xl
                        ${
                          isActive
                            ? "bg-white border-b-4 border-r-4 border-[#247994]"
                            : "bg-white/80"
                        }`}
                    >
                      <div
                    ...
                    {isActive && (
                      <div className="bg-[#247994]/5 rounded-2xl p-4 ml-12 mt-3 border-l-2 border-[#247994]">
                        {item.places.map((place, i) => (
                          <div
                            key={i}
                            className="flex items-center gap-3 py-1.5 text-slate-700"
                          >
                            <Navigation
                              size={12}
          ...
          {/* RIGHT – GOOGLE MAP (NO API KEY) */}
          <div className="lg:col-span-7">
            <div className="relative w-full aspect-[4/5] sm:aspect-video lg:aspect-square max-h-[600px] rounded-3xl overflow-hidden shadow-2xl">
              <iframe
                title="Location Map"
                loading="lazy"
                className="w-full h-full border-0"
                referrerPolicy="no-referrer-when-downgrade"
                src="https://www.google.com/maps?q=Aakash+Nirvana+Premium+Residential,+Mango,+Jamshedpur,+Jharkhand&output=embed"
              />
            </div>
          </div>
        </div>
